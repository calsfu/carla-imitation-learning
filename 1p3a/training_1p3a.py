...
from network_1p3a import ClassificationNetwork

from dataset_1p3a import get_dataloader
...
def train(data_folder, save_path):
    ...
    for epoch in range(nr_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            batch_image, batch_sensor, batch_gt = batch[0].to(gpu), batch[1].to(gpu), batch[2].to(gpu)

            batch_out = infer_action(batch_image, batch_sensor)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--save_path', default="./hw1/1p3a/model_1p3a.pth", type=str, help='path where to save your model in .pth format')
