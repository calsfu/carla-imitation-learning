...
class CarlaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return    (image, sensors, action), both in torch.Tensor format
        """
        data = np.load(self.data_list[idx], allow_pickle=True).item()

        observation = data['observation']
        control = data['action']
        sensors = [observation['gnss'][0], observation['gnss'][1], *observation['imu'][0], *observation['imu'][1], observation['imu'][2]]
