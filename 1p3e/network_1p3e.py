...
class ClassificationNetwork(torch.nn.Module):
    def __init__(self):
        ...
        observations is 320x240 pixels in 3 channels and 3 frames.
        """
        super().__init__()
        gpu = torch.device('cuda')
        self.model = torch.nn.Sequential(
            nn.Conv3d(in_channels=3, out_channels=24, kernel_size=(1, 5, 5), stride=(1, 2, 2)),  # Conv3d for 3 frames
            nn.ReLU(),
            nn.Conv3d(in_channels=24, out_channels=36, kernel_size=(1, 5, 5), stride=(1, 2, 2)),
            nn.ReLU(),
            nn.Conv3d(in_channels=36, out_channels=48, kernel_size=(1, 5, 5), stride=(1, 2, 2)),
            nn.ReLU(),
            nn.Conv3d(in_channels=48, out_channels=64, kernel_size=(1, 3, 3), stride=(1, 2, 2)),
            nn.ReLU(),
            nn.Conv3d(in_channels=64, out_channels=64, kernel_size=(1, 3, 3), stride=(1, 1, 1)),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(33792, 512),  
            nn.ReLU(),
